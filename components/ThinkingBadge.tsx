...
export default function ThinkingBadge({ message }: Props) {
  ...
  const isEvaluating = message?.status === 'heartbeat' || message?.isEvaluatingComplexity
    ...
    <AnimatePresence mode="wait">
      {isEvaluating ? (
        <motion.div
          ...
          className="flex items-center min-h-[32px] mb-5 select-none pl-[44px]"
        >
          {/* Simple ChatGPT-style neutral processing dot */}
          <motion.div
            className="w-5 h-5 rounded-full bg-[#BDBDBD] shrink-0"
            animate={{
              scale: [0.8, 1.12, 0.8],
            }}
            transition={{
        ...
        <motion.div
          ...
          className="flex items-start gap-3 mb-5 select-none"
        >
          {/* ─── Icon ─── */}
